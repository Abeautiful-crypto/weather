...
func newEnvWithEndpoints(t *testing.T, ttl, timeout time.Duration, eps upstream.Endpoints) string {
	...
	srv := New(cache.New(ttl), upstream.New(timeout, eps), nil, nil, logger)
...
func TestStaticDirectoryListingIsDisabled(t *testing.T) {
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	srv := New(cache.New(time.Minute), upstream.New(time.Second, upstream.DefaultEndpoints()), nil, nil, logger)
	static := fstest.MapFS{
...
func TestGeocodeCandidates(t *testing.T) {
	cases := []struct {
		...
		{"南阳", []string{"南阳", "南阳市"}},
		{"南阳市", []string{"南阳市", "南阳"}},          // 去掉后缀的变体：南阳市 → 南阳
		{"淅川县", []string{"淅川县", "淅川"}},          // 县级地名靠"去掉县"才能命中
		{"宿城区", []string{"宿城区", "宿城"}},          // 区同样会剥离后缀
